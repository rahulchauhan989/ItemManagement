...
namespace ItemManagementSystem.Application.Implementation
{
    public class ItemTypeService : IItemTypeService
    {
        ...
        public async Task<ItemTypeCreateRequest> CreateAsync(ItemTypeCreateRequest dto, int userId)
        {
            ...
            var exists = (await _itemTypeRepository.FindAsync(
            ...
            var created = await _itemTypeRepository.AddAsync(entity);
        ...
        public async Task<PagedResultDto<ItemTypePagedDto>> GetPagedItemTypesAsync(ItemTypeFilterDto filter)
        {
            ...
            var pagedResult = await _itemTypeRepository.GetPagedWithMultipleFiltersAndSortAsync(
                filterProperties,
        ...
        public async Task<ItemTypeCreateRequest> updateAsync(int id, ItemTypeCreateRequest dto, int userId)
        {
            ...
            await _itemTypeRepository.UpdateAsync(entity);
        ...
        public async Task DeleteAsync(int id)
        {
            ...
            if (entity.IsDeleted)
                throw new CustomException(AppMessages.ItemTypeAlreadyDeleted);

            var hasAssociatedModels = (await _itemmodel.FindAsync(
                im => im.ItemTypeId == id && im.IsDeleted == false
            )).Any();
